...
public class ViewEventController {
    ...
    public void initialize(){
        updateGUI();
        memoListView.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                if(memoListView.getSelectionModel().getSelectedItem() != null){
                    int index = memoListView.getSelectionModel().getSelectedIndex();
                    sys.selectMemo(index);
                    try{viewSpecificMemo();}catch (IOException e){e.printStackTrace();}
                }}
        });

        alertListView.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                if(alertListView.getSelectionModel().getSelectedItem() != null){
                    int index = alertListView.getSelectionModel().getSelectedIndex();
                    sys.selectAlert(index);
                    try{viewSpecificAlert();}catch (IOException e){e.printStackTrace();}
                }}
        });
        ...
        tagListView.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                if(tagListView.getSelectionModel().getSelectedItem() != null){
                    ...
                    sys.selectTag(index);
                    sys.searchByTag(tagListView.getSelectionModel().getSelectedItem());
                    try{viewSpecificTag();}catch (IOException e){e.printStackTrace();}
                }}
        });
    }

    private void viewSpecificTag() throws IOException{
        Stage primaryStage = new Stage();
        Parent root = FXMLLoader.load(getClass().getResource("../view_tag/ViewTag.fxml"));
        primaryStage.setScene(new Scene(root));
        primaryStage.initOwner(confirm.getScene().getWindow());
        primaryStage.initModality(Modality.APPLICATION_MODAL);
        primaryStage.showAndWait();
        updateGUI();
    }

    private void viewSpecificAlert() throws IOException{
        Stage primaryStage = new Stage();
        Parent root = FXMLLoader.load(getClass().getResource("../view_alert/ViewAlert.fxml"));
        primaryStage.setScene(new Scene(root));
        primaryStage.initOwner(confirm.getScene().getWindow());
        primaryStage.initModality(Modality.APPLICATION_MODAL);
        primaryStage.showAndWait();
    ...
    private void viewSpecificMemo() throws IOException{
        Stage primaryStage = new Stage();
        Parent root = FXMLLoader.load(getClass().getResource("../view_memo/ViewMemo.fxml"));
        primaryStage.setScene(new Scene(root));
        primaryStage.initOwner(confirm.getScene().getWindow());
        primaryStage.initModality(Modality.APPLICATION_MODAL);
        primaryStage.showAndWait();
